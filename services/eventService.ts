import { Event, RecurrenceFrequency, DisplayEvent } from "../types";
...
export const getEventsForDateRangeOptimized = (
  events: Event[],
  viewStartDate: Date,
  viewEndDate: Date
): DisplayEvent[] => {
  ...
  events.forEach((event) => {
    ...
    if (
      ...
      event.recurrenceRule.frequency !== RecurrenceFrequency.NONE
    ) {
      const {
        frequency,
        interval = 1,
      ...
      while (
        isBefore(currentDate, maxIterationDate) &&
        iterations < maxIterations
      ) {
        iterations++;
        if (recurrenceEndDate && isAfter(currentDate, recurrenceEndDate)) break;
        if (
          isAfter(currentDate, addMonths(viewEndDate, 1)) &&
          isAfter(originalEventStart, viewEndDate)
        )
          break;

        if (
          isAfter(currentDate, originalEventStart) ||
          isSameDay(currentDate, originalEventStart)
        ) {
          ...
          if (isValidOccurrence && !isOccurrenceCancelled(event, currentDate)) {
            ...
            instanceSpecificStartDt.setHours(
              originalEventStart.getHours(),
              originalEventStart.getMinutes(),
              originalEventStart.getSeconds(),
              originalEventStart.getMilliseconds()
            );
            const instanceSpecificEndDt = new Date(
              instanceSpecificStartDt.getTime() + duration
            );

            if (
              isWithinInterval(instanceSpecificStartDt, {
                start: startOfDay(viewStartDate),
                end: endOfDay(viewEndDate),
              }) ||
              isWithinInterval(instanceSpecificEndDt, {
                start: startOfDay(viewStartDate),
                end: endOfDay(viewEndDate),
              }) ||
              (isBefore(instanceSpecificStartDt, viewStartDate) &&
                isAfter(instanceSpecificEndDt, viewEndDate))
            ) {
              displayEvents.push({
                ...event,
      ...
      // Non-recurring event
      if (
        isWithinInterval(originalEventStart, {
          start: startOfDay(viewStartDate),
          end: endOfDay(viewEndDate),
        }) ||
        isWithinInterval(originalEventEnd, {
          start: startOfDay(viewStartDate),
          end: endOfDay(viewEndDate),
        }) ||
        (isBefore(originalEventStart, viewStartDate) &&
          isAfter(originalEventEnd, viewEndDate))
      ) {
  ...
  const uniqueDisplayEvents = Array.from(
    new Map(displayEvents.map((e) => [`${e.id}-${e.start}`, e])).values()
  );
  return uniqueDisplayEvents.sort(
...
export const checkConflict = (
  ...
  if (recurrenceRule && recurrenceRule.frequency !== RecurrenceFrequency.NONE) {
    const instances = getEventsForDateRangeOptimized(
      [eventToCheck],
      viewStartDate,
      viewEndDate
    );
    ...
    potentialInstancesToCheck.push({
      ...
      instanceDate:
        eventToCheck.instanceDate ||
        format(parseISO(newStartStr), "yyyy-MM-dd"),
      isInstance: !!eventToCheck.isInstance,
    });
  }
  ...
  for (const pInstance of potentialInstancesToCheck) {
    ...
    for (const existingEventInstance of allExistingDisplayEvents) {
      ...
      if (
        isBefore(pInstanceStart, existingEnd) &&
        isAfter(pInstanceEnd, existingStart)
      ) {
        return true;
      }
    }
